#!/usr/bin/env python
# coding: utf-8
##Replace names of .pkl files for train_val in build_graph.py line 91,92,93
import os
import glob
...
class SGConstructor:
    ...
    def from_dict(cls, graph_file):
        dict_dir = 'data/dicts' ##'../../data/dicts/'
        attribute_category = DataIO.from_json(os.path.join(dict_dir, 'attributeCategoryExtend.json'))

        ind_to_attr = DataIO.from_pickle(os.path.join(dict_dir, 'train10k_val_gt_attr.pkl'))
        ind_to_cls = DataIO.from_pickle(os.path.join(dict_dir, 'train10k_val_gt_class.pkl'))
        ind_to_rel = DataIO.from_pickle(os.path.join(dict_dir, 'train10k_val_gt_rels.pkl'))
    ...
    def instance_from_dict(img_shape, graph_dict, attribute_category=None, topk=2, attr=True, ind_to_cls=None, ind_to_rel=None, ind_to_attr=None):
        ...
        for idx in range(len(obj_score)):
            k = f"x{idx}"
            #print("obj_score:",obj_score[idx])
            #print("ind_to_class:",ind_to_cls)
        ...
        for idx, rel in enumerate(graph_dict['pred_rels']):
            ...
            #print("len(pred_rels)",len(graph_dict['pred_rels']))
            #print("len(gt_rels)",len(graph_dict['gt_rels']))
            #print("len(rel_scores)",len(graph_dict['rel_scores']))
            #print("idx",idx)
            #print('Eneumerate',[(idx,rel) for idx, rel in enumerate(graph_dict['pred_rels'])]) ###vector form
            #print(rel_score[idx])
            #print("Vocab",vocab[tgt])
            link = {'id': vocab[tgt],
...
if __name__=='__main__':
    ...
    output_dir = 'codes/preprocessing/scenegraphs/graphs'
